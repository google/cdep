...
class Variant {
 public:
  ...
  /// will return true.
  ///
  /// @param[in] value The string to use for the the Variant.
  ...
  template <typename T>
  Variant(const T array_of_values[], size_t size) : type_(kTypeNull) {
    Clear(kTypeVector);
    vector().reserve(size);
  ...
  /// empty strings, empty vectors, empty maps, and "false" (case-sensitive) are
  /// all considered false. All other values are true.
  ///
  ...
  std::string& mutable_string() {
    if (type_ == kTypeStaticString) {
      ...
      const char* prev_string = string_value();
      set_mutable_string(prev_string);
    }
  ...
  static const char* const TypeName(Type type);
